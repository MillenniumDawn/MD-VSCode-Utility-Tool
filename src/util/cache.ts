...
export class Cache<V> {
    ...
    protected enforceLimits(): void {
        ...
        if (maxBytes !== undefined) {
            for (const k of keys) {
                totalBytes += this._cache[k].weight;
            }
        }

        const over = () => (maxSize !== undefined && count > maxSize) || (maxBytes !== undefined && totalBytes > maxBytes);
        if (!over()) {
            return;
        }
        ...
        if (this.pweigher && this.options.maxBytes !== undefined) {
            value.then(v => {
                if (this._cache[key] === newEntry) {
                    newEntry.weight = (v === null || v === undefined) ? 0 : (this.pweigher!(v) ?? 0);
                    this.enforceLimits();
                }
