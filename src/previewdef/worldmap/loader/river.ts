...
async function loadRivers(
	...
	await progressReporter(
		localize("worldmap.progress.loadingrivers", "Loading rivers..."),
	);

	const [riversImageBuffer] = await readFileFromModOrHOI4(file);
	const riversImage = parseBmp(
		riversImageBuffer.buffer as ArrayBuffer,
		riversImageBuffer.byteOffset,
	);
...
function findRiverPointsList(riversImage: BMP): River[] {
	...
	for (
		let y = riversImage.height - 1,
			sy = 0,
			dy = (riversImage.height - 1) * riversImage.width;
		y >= 0;
...
function findRiverPoints(
	...
	while (stack.length > 0) {
		...
		if (
			y > 0 &&
			(riversImage.data[si + riversImage.bytesPerRow] ?? 255) <= 11
		) {
			stack.push({ x, y: y - 1 });
			adjecents++;
		}
		if (
			y < riversImage.height - 1 &&
			(riversImage.data[si - riversImage.bytesPerRow] ?? 255) <= 11
		) {
			stack.push({ x, y: y + 1 });
			adjecents++;
	...
	for (const key in colors) {
		...
		convertedColors[(y - boundingBox.y) * boundingBox.w + (x - boundingBox.x)] =
			value;
	}
	...
	for (const end of ends) {
		...
		convertedEnds.push(
			(y - boundingBox.y) * boundingBox.w + (x - boundingBox.x),
		);
	}
...
function validateRiver(
	...
	index: number,
	river: River,
	warning: WorldMapWarning[],
) {
	if (river.ends.length === 0) {
		warning.push({
			relatedFiles: [file],
			text: localize(
				"worldmap.warning.rivernoends",
				"River has no end points.",
			),
	...
	if (sources.length === 0) {
		warning.push({
			...
			text: localize(
				...
				river.ends.map((e) => riverToString(river, e)).join(", "),
			),
			source: [
				{
					type: "river",
					name: riverToString(river, river.ends[0] ?? 0),
					index: index,
				},
			],
		});
	...
	if (sources.length > 1) {
		warning.push({
			relatedFiles: [file],
			text: localize(
				"worldmap.warning.rivermultiplesource",
				"River has multiple sources: {0}.",
				sources.map((s) => riverToString(river, s)).join(", "),
			),
			source: [
				{ type: "river", name: riverToString(river, sources[0]), index: index },
			],
		});
	}
			...
			if (next === -1) {
				...
				warning.push({
					...
					text: localize(
						...
						riverToString(river, current),
					),
					source: [
						{ type: "river", name: riverToString(river, end), index: index },
					],
				});
				return;
			}
		...
		if (next === -1) {
			if (!adjecentToMark) {
				warning.push({
					relatedFiles: [file],
					text: localize(
						"worldmap.warning.rivermayloop",
						"River may contain a loop at {0} ~ {1}.",
						riverToString(river, end),
						riverToString(river, current),
					),
					source: [
						{ type: "river", name: riverToString(river, end), index: index },
