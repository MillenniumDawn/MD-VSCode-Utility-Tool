...
export class DefaultMapLoader extends FileLoader<ProvinceMap> {
	...
	constructor() {
		...
		this.terrainDefinitionLoader.onProgress((e) =>
			this.onProgressEmitter.fire(e),
		);
	}
		...
		return (
			await Promise.all(
				[
					...
					this.riverLoader,
				].map((v) => v?.shouldReload(session) ?? Promise.resolve(false)),
			)
		).some((v) => v);
	}
	...
	protected async loadFromFile(
	...
	): Promise<LoadResult<ProvinceMap>> {
		const defaultMap = await loadDefaultMap((e) => this.fireOnProgressEvent(e));
		session.throwIfCancelled();

		const provinceDefinitions = await (this.definitionsLoader =
			this.checkAndCreateLoader(
				this.definitionsLoader,
		...
		const provinceBmp = await (this.provinceBmpLoader =
			this.checkAndCreateLoader(
				this.provinceBmpLoader,
				"map/" + defaultMap.provinces,
				ProvinceBmpLoader,
			)).load(session);
		session.throwIfCancelled();

		const adjacencies = await (this.adjacenciesLoader =
			this.checkAndCreateLoader(
				this.adjacenciesLoader,
				"map/" + defaultMap.adjacencies,
				AdjacenciesLoader,
			)).load(session);
		session.throwIfCancelled();

		const continents = await (this.continentsLoader = this.checkAndCreateLoader(
			this.continentsLoader,
			"map/" + defaultMap.continent,
			ContinentsLoader,
		)).load(session);
		session.throwIfCancelled();
		...
		const rivers = await (this.riverLoader = this.checkAndCreateLoader(
			this.riverLoader,
			"map/" + defaultMap.rivers,
			RiverLoader,
		)).load(session);
		session.throwIfCancelled();

		const subLoaderResults = [
			provinceDefinitions,
			provinceBmp,
			adjacencies,
			continents,
		...
		const { provinces, badProvinceId: badProvinceIdForMerge } =
			mergeProvinceDefinitions(
				provinceDefinitions.result,
				provinceBmp.result,
				["map/" + defaultMap.definitions, "map/" + defaultMap.provinces],
				warnings,
			);

		validateProvinceContinents(
			provinces,
			continents.result,
			["map/" + defaultMap.definitions, "map/" + defaultMap.continent],
			warnings,
		);
		validateProvinceTerrains(
			provinces,
			terrains.result,
			["map/" + defaultMap.definitions],
			warnings,
		...
		fillAdjacencyEdges(
			...
			["map/" + defaultMap.provinces, "map/" + defaultMap.definitions],
			warnings,
		);
		...
		if (
			rivers.result.width !== provinceBmp.result.width ||
			rivers.result.height !== provinceBmp.result.height
		) {
			warnings.push({
...
async function loadDefaultMap(
	...
	await progressReporter(
		localize("worldmap.progress.loadingdefaultmap", "Loading default.map..."),
	);

	const defaultMap = await readFileFromModOrHOI4AsJson<DefaultMap>(
		"map/default.map",
		defaultMapSchema,
	);
	(
	...
	const { sorted, badId } = sortItems(
		...
		(maxId) => {
			throw new UserError(
				localize(
					...
					"Max province id is too large: {0}.",
					maxId,
				),
			);
		},
		(newProvince, existingProvince, badId) =>
			warnings.push({
				source: [
					{ type: "province", id: badId, color: existingProvince.color },
				],
				relatedFiles,
				text: localize(
		...
		(startId, endId) =>
			warnings.push({
				...
				relatedFiles: [],
				text: localize(
					"worldmap.warnings.provincenotexist",
					"Province with id {0} doesn't exist.",
...
function mergeProvinceDefinitions(
	...
	for (const provinceDef of provinceDefinitions) {
		if (colorToProvinceId[provinceDef.color] !== undefined) {
			warnings.push({
				source: [
					provinceDef.id,
					colorToProvinceId[provinceDef.color] ?? null,
				].map((id) => ({ type: "province", id, color: provinceDef.color })),
				relatedFiles: relatedFiles.slice(0, 1),
				text: localize(
					"worldmap.warnings.provincecolorconflict",
					"Province {0} has conflict color with province {1}.",
					provinceDef.id,
					colorToProvinceId[provinceDef.color],
				),
			});
		}
		...
		if (provinceInMap) {
			...
			if (provinceDef.id !== 0) {
				warnings.push({
					source: [
						{ type: "province", id: provinceDef.id, color: provinceDef.color },
					],
	...
	for (const province of result) {
		...
		if (provinceInMap) {
			province.edges = provinceInMap.edges.map((e) => ({
				...e,
				to: colorToProvinceId[e.toColor] ?? -1,
				type: "",
			}));
		}
	}
...
function validateProvinceContinents(
	provinces: Province[],
	continents: string[],
	relatedFiles: string[],
	warnings: WorldMapWarning[],
	...
	for (const province of provinces) {
		...
		if (continent >= continents.length || continent < 0) {
			warnings.push({
				...
				relatedFiles,
				text: localize(
					"worldmap.warnings.continentnotdefined",
					"Continent {0} is not defined.",
					continent,
				),
			});
		}
		if (
			province.type === "land" &&
			(continent === 0 || Number.isNaN(continent)) &&
			province.id !== 0
		) {
			warnings.push({
				source: [
...
function validateProvinceTerrains(
	provinces: Province[],
	terrains: Terrain[],
	relatedFiles: string[],
	warnings: WorldMapWarning[],
	...
	for (const province of provinces) {
		...
		if (!terrainObj) {
			warnings.push({
				...
				relatedFiles,
				text: localize(
					"worldmap.warnings.terrainnotdefined",
					'Terrain "{0}" is not defined.',
					terrain,
				),
			});
		}
...
function fillAdjacencyEdges(
	provinces: (Province | undefined)[],
	adjacencies: ProvinceEdgeAdjacency[],
	height: number,
	relatedFiles: string[],
	warnings: WorldMapWarning[],
) {
	for (const {
		...
		const resultThrough =
			through !== undefined && !Number.isNaN(through) && through !== -1
				? through
				: undefined;
		...
		const start = saveStart
			? { ...saveStart, y: height - saveStart.y }
			: undefined;
