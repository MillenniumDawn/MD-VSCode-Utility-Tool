...
async function loadStrategicRegion(
	file: string,
	globalWarnings: WorldMapWarning[],
): Promise<StrategicRegionNoRegion[]> {
	const result: StrategicRegionNoRegion[] = [];
	try {
		const data = await readFileFromModOrHOI4AsJson<StrategicRegionFile>(
			file,
			strategicRegionFileSchema,
		);
		for (const strategicRegion of data.strategic_region) {
			const warnings: string[] = [];
			const id = strategicRegion.id
				? strategicRegion.id
				: (warnings.push(
						localize(
					...
					-1);
			const name = strategicRegion.name
				? strategicRegion.name
				: (warnings.push(
						localize(
					...
					"");
			const provinces = strategicRegion.provinces._values.map((v) =>
				parseInt(v, 10),
			);
...
function validateStrategicRegions(
...
function validateProvincesInStrategicRegions(
			...
			if (provinceToStrategicRegion[p] !== undefined) {
				...
				warnings.push({
					source: [
						...[
							strategicRegion.id,
							provinceToStrategicRegion[p],
						].map<WorldMapWarningSource>((id) => ({
							type: "strategicregion",
							id,
						})),
	...
	for (let i = -badStatesCount; i < states.length; i++) {
		...
		strategicRegionId.forEach(
			([_, sr]) =>
				(strategicRegionIdCount[sr] = (strategicRegionIdCount[sr] ?? 0) + 1),
		);
		const entries = Object.entries(strategicRegionIdCount);
		if (entries.length > 1) {
			entries.sort((a, b) => b[1] - a[1]);
			const mostStrategicRegionId = parseInt(entries[0]?.[0] ?? "0", 10);
			const badProvinces = strategicRegionId
				.filter(([_, sr]) => sr !== mostStrategicRegionId)
				.map((v) => v[0]);
			warnings.push({
				source: [
					...badProvinces.map<WorldMapWarningSource>((id) => ({
						type: "province",
						id,
						color: provinces[id]?.color ?? -1,
					})),
					{ type: "state", id: i },
				],
