...
> {
	...
	protected async mergeFiles(
		...
		await this.fireOnProgressEvent(
			localize(
				"worldmap.progress.mapstatetosupplyarea",
				"Mapping states to supply areas...",
			),
		);
...
): Promise<SupplyAreaNoRegion[]> {
	const result: SupplyAreaNoRegion[] = [];
	try {
		const data = await readFileFromModOrHOI4AsJson<SupplyAreaFile>(
			file,
			supplyAreaFileSchema,
		);
		for (const supplyArea of data.supply_area) {
			const warnings: string[] = [];
			const id = supplyArea.id
				? supplyArea.id
				: (warnings.push(
						localize(
...
function validateStatesInSupplyAreas(
	...
	for (let i = -badSupplyAreasCount; i < supplyAreas.length; i++) {
		...
		const statesInSupplyArea = supplyArea.states
			.map((s) => {
				...
				if (stateToSupplyArea[s] !== undefined) {
					...
					warnings.push({
						...
						relatedFiles: [
							supplyArea.file,
							supplyAreas[stateToSupplyArea[s]]?.file ?? "",
							state.file,
						],
						text: localize(
	...
	for (let i = 1; i < states.length; i++) {
		...
		if (!(i in stateToSupplyArea)) {
			warnings.push({
				...
				text: localize(
					"worldmap.warnings.statenosupplyarea",
					"State {0} is not in any supply area.",
					i,
				),
			});
		}
...
function checkStatesContiguous(
	states: State[],
	provinces: (Province | undefined | null)[],
	...
	return inAccessedState === undefined
		? undefined
		: [inAccessedState.id, parseInt(Object.keys(accessedStates)[0] ?? "0", 10)];
}

function statesAreAdjacent(
	stateA: State,
	stateB: State,
	provinces: (Province | undefined | null)[],
): boolean {
	return stateA.provinces.some(
		(p) =>
			provinces[p]?.edges.some(
				(e) =>
					e.type !== "impassable" &&
					stateB.provinces.some((p2) => provinces[p2] && e.to === p2),
			) ?? false,
	);
}
