...
export abstract class FileLoader<T> extends CommonFileLoader<
	...
	MapLoaderExtra
> {}
export abstract class FolderLoader<T, F> extends CommonFolderLoader<
	T,
	F,
	MapLoaderExtra,
	MapLoaderExtra
...
export async function shouldReloadDependencies<T>(
	...
	return (
		await Promise.all(loaders.map((loader) => loader.shouldReload(session)))
	).some(Boolean);
}
...
export function mergeRegions(
	...
	const nearBorder = regions
		.map((r) => ("mass" in r ? r.boundingBox : r))
		.every((z) => z.w + z.x < oneFourthWidth || z.x > threeFourthWidth);
	...
	for (const region of regions) {
		...
		if ("mass" in region) {
			massX +=
				(region.centerOfMass.x +
					(nearBorder && region.centerOfMass.x > halfWidth ? -width : 0)) *
				region.mass;
			...
			massX +=
				(region.x +
					region.w / 2 +
					(nearBorder && region.x + region.w / 2 > halfWidth ? -width : 0)) *
				regionMass;
		...
		minX = Math.min(
			minX,
			regionBondingBox.x +
				(nearBorder && regionBondingBox.x > halfWidth ? -width : 0),
		);
