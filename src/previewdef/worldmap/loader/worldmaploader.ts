...
export class WorldMapLoader extends Loader<WorldMapData> {
	...
	constructor() {
		...
		this.statesLoader = new StatesLoader(
		...
		this.strategicRegionsLoader = new StrategicRegionsLoader(
			this.defaultMapLoader,
			this.statesLoader,
		);
		this.strategicRegionsLoader.onProgress((e) =>
			this.onProgressEmitter.fire(e),
		);

		this.supplyAreasLoader = new SupplyAreasLoader(
			this.defaultMapLoader,
			this.statesLoader,
		);
	...
	public async loadImpl(
	...
	): Promise<LoadResult<WorldMapData>> {
		...
		const supplyAreas = enableSupplyArea
			...
			: {
					warnings: [],
					result: { supplyAreas: [], badSupplyAreasCount: 0 },
					dependencies: [],
				};
		session.throwIfCancelled();
