...
import { getFocusTitlebarImage, getFocusOverlayImage, loadFocusTitlebarStyles, nationalFocusViewGfxFile } from "./titlebar";
...
import { ExclusiveLinkImages, registerExclusiveLinkStyles } from "./exclusivelinkstyles";
...
export async function buildFocusTreePayload(loader: FocusTreeLoader, progress?: ProgressCallback, options?: { resolveIcons?: boolean }): Promise<FocusTreePayload | null> {
    ...
    try {
        ...
        registerExclusiveLinkStyles(styleTable, resolveIcons ? await loadExclusiveLinkImages() : undefined);
...
// Sprite names and frames as `national_focus_exclusive_item` in `nationalfocusview.gui` uses them.
// A `frame = 1` there is the first frame; the sprite frame array is 0 based.
...
export async function loadExclusiveLinkImages(): Promise<ExclusiveLinkImages | undefined> {
    // Both sprites live in nationalfocusview.gfx, which the focus tree loader already depends on.
