...
import { buildFocusTreePayload, buildFocusTreeHtml, buildNoFocusTreeHtml, buildFocusTreeErrorHtml, loadFocusTreesOnly, focusTreeGridBox } from '../previewdef/focustree/contentbuilder';
import { StyleTable } from '../util/styletable';

const webview = { asWebviewUri: (u: unknown) => u, cspSource: 'test-csp' } as unknown as vscode.Webview;
    ...
    return {
        ...
        focuses: {
            focus_a: {
                ...
                text: undefined, textIcon: undefined, overlay: undefined,
...
describe('previewdef/focustree contentbuilder', () => {
    it('buildFocusTreePayload returns null for empty trees', async () => {
        const payload = await buildFocusTreePayload(loaderWithTrees([]), undefined, { resolveIcons: false });
        assert.strictEqual(payload, null);
    });

    it('buildFocusTreePayload handles loader error as null', async () => {
        const badLoader: any = { file: 'test.txt', load: async () => { throw new Error('boom'); } };
        const payload = await buildFocusTreePayload(badLoader, undefined, { resolveIcons: false });
        assert.strictEqual(payload, null);
    });
    ...
    it('buildFocusTreePayload reports progress', async () => {
        ...
        const payload = await buildFocusTreePayload(loaderWithTrees([tree]), progress, { resolveIcons: false });
        assert.ok(payload);
    ...
    it('buildNoFocusTreeHtml contains localized placeholder', () => {
        ...
        assert.ok(html.includes('No focus tree') || html.includes('nofocustree') || html.length > 0);
    });
    ...
    it('buildFocusTreeErrorHtml contains error and reload button', () => {
        ...
        assert.ok(html.includes('Error') || html.includes('test error'));
        assert.ok(html.includes('ft-reload'));
    ...
    it('renders multiple focuses', async () => {
        const tree: any = {
            id: 'multi',
            focuses: {
                a: { id: 'a', x: 0, y: 0, icon: [{ icon: 'GFX_a' }], token: { start: 0, end: 5 }, file: 'test.txt' },
                b: { id: 'b', x: 1, y: 0, icon: [{ icon: 'GFX_b' }], token: { start: 6, end: 10 }, file: 'test.txt' },
            },
            inlayWindows: [], inlayWindowRefs: [], warnings: [], allowBranchOptions: [],
        };
        const payload = await buildFocusTreePayload(loaderWithTrees([tree]), undefined, { resolveIcons: false });
        assert.ok(payload);
