...
suite("Integration smoke", () => {
	test("extension is present", async () => {
		const ext = vscode.extensions.getExtension(
			"MilleniumDawnModTeam.hearts-of-iron-iv-utilities-2026",
		);
	...
	test("extension activates and registers commands", async () => {
		...
		for (const id of [
			"mdhoi4utilities.preview",
			"mdhoi4utilities.previewworld",
			"mdhoi4utilities.selectmodfile",
			"mdhoi4utilities.selecthoifolder",
		]) {
	...
	test("custom editors are registered", async () => {
		...
		const viewTypes: string[] = (manifest.contributes?.customEditors ?? []).map(
			(e: any) => e.viewType,
		);
		assert.ok(
			viewTypes.includes("mdhoi4utilities.dds"),
			"dds custom editor not contributed",
		);
		assert.ok(
			viewTypes.includes("mdhoi4utilities.tga"),
			"tga custom editor not contributed",
		);
	});
});
