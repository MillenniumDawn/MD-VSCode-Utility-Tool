...
describe('hoiformat/effect', function () {
    ...
    describe('projectEffects group merging', function () {
        function project(text: string): EffectTreeNode[] {
            return projectEffects(extractEffectValue(parseHoi4File(text).value, countryScope).effect);
        }
        ...
        it('merges statements that share a condition into one group, in source order', function () {
            ...
            const nodes = project(['a = 1', 'hidden_effect = { b = 2 }', 'c = 3', 'hidden_effect = { d = 4 }'].join('\n'));
        ...
        it('merges a second block under the same condition object into the first group', function () {
            // The else of an if and the next if share nothing; but two hidden_effect blocks
            // inside one if share the if's condition object, so their statements join one group.
            const nodes = project([
                'if = {',
                '    limit = { tag = GER }',
                '    hidden_effect = { a = 1 }',
            ...
            const [only] = nodes as Extract<EffectTreeNode, { kind: 'group' }>[];
            assert.deepStrictEqual(lines(only!.items), ['a = 1', 'b = 2']);
        });
    });
});
