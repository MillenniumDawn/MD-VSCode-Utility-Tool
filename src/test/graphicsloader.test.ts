...
import { getImageByPath, getSpriteByGfxName, _clearImageCachesForTest } from '../util/image/imagecache';
import { _setImageWorkerPathForTest, _terminateImageWorkerForTest } from '../util/image/imagedecoder';
...
describe('util/image/graphicsloader (headless)', function () {
    ...
    it('passes a PNG through without re-encoding', async function () {
        ...
        assert.ok(image!.pngBuffer.equals(png), 'PNG passthrough should keep the same buffer');
    });
    ...
    it('decodes many textures concurrently through the worker pool', async function () {
        ...
        const images = await Promise.all(
            Array.from({ length: 8 }, (_, i) => getImageByPath(`gfx/interface/goals/icon${i}.dds`)),
        );
