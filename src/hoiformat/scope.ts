...
export function tryMoveScope(
	...
	if (nodeName === "root") {
		scopeStack.push(scopeStack[0] ?? countryScope);
		return true;
	}
	...
	if (variableMatch) {
		...
		scopeStack.push({
			scopeName: global
				? "{" + nodeName + "}"
				: currentScope.scopeName + ".{" + nodeName + "}",
			scopeType: "unknown",
		});
		return true;
	}
	...
	if (
		scopeDef &&
		((type === "condition" && scopeDef.condition) ||
			(type === "effect" && scopeDef.effect))
	) {
		if (scopeDef.from === "*") {
			scopeStack.push({
			...
			scopeDef.from === currentScope.scopeType ||
			currentScope.scopeType === "unknown"
		) {
			scopeStack.push({
...
export const scopeDefs = arrayToMap(
	[
		...
		scopeDef(
			"all_military_purchase_contract",
			true,
			false,
			"country",
			"purchaseContract",
		...
		scopeDef(
			"any_military_purchase_contract",
			true,
			false,
			"country",
			"purchaseContract",
		...
		scopeDef(
			"every_military_purchase_contract",
			false,
			true,
			"country",
			"purchaseContract",
		...
		scopeDef(
			"random_military_purchase_contract",
			false,
			true,
			"country",
			"purchaseContract",
