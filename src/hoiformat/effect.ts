...
export function extractEffectValue(
	nodeValue: NodeValue,
	scope: Scope,
	excludedKeys: string[] | undefined = undefined,
): EffectValue {
	const effect = simplifyEffect(
		extractEffectByCondition(nodeValue, [scope], true, [], excludedKeys),
	);
	return {
		effect,
	};
...
function handleIf(
	...
	limit: Node,
	scopeStack: Scope[],
	baseCondition: ConditionComplexExpr,
	result: EffectComplexExpr[],
): ConditionFolder {
...
function simplifyEffect(effect: EffectComplexExpr): EffectComplexExpr {
	...
	if ("condition" in effect) {
		...
		if (effect.condition === true) {
			...
			if (onlyItem !== undefined && items.length === 1) {
				return simplifyEffect(onlyItem);
			}
		}
		...
		const onlyItem = items[0];
		if (onlyItem !== undefined && items.length === 1) {
			return simplifyEffect(onlyItem.effect);
		}
