...
const worldmapview_html_1 = tslib_1.__importDefault(require("./worldmapview.html"));
const worldmapview_css_1 = tslib_1.__importDefault(require("./worldmapview.css"));
...
class WorldMap {
    panel;
    worldMapLoader;
    worldMapDependencies;
    cachedWorldMap;
    lastRequestedExportUri;
    constructor(panel) {
        this.panel = panel;
    ...
    onDocumentChange = (0, common_1.debounceByInput)((uri) => {
        ...
        try {
            ...
            switch (msg.command) {
                ...
                case 'openfile':
                    ...
                    break;
                case 'telemetry':
                    await (0, telemetry_1.sendByMessage)(msg);
                    break;
    ...
    async sendProvinceMapSummaryToWebview(force) {
        try {
    ...
    async openFile(file, type, start, end) {
        ...
        if (!vscode.workspace.workspaceFolders?.length) {
            await vscode.window.showErrorMessage((0, i18n_1.localize)('worldmap.mustopenafolder', 'Must open a folder before opening {0} file.', typeName));
            return;
        }
        let targetFolderUri = vscode.workspace.workspaceFolders[0].uri;
        if (vscode.workspace.workspaceFolders.length >= 1) {
            const folder = await vscode.window.showWorkspaceFolderPick({ placeHolder: (0, i18n_1.localize)('worldmap.selectafolder', 'Select a folder to copy {0} file', typeName) });
            if (!folder) {
                return;
            }
        ...
        try {
            ...
            await (0, vsccommon_1.mkdirs)((0, vsccommon_1.dirUri)(targetPath));
            await (0, vsccommon_1.writeFile)(targetPath, buffer);
            ...
            await vscode.window.showErrorMessage((0, i18n_1.localize)('worldmap.failedtoopenstate', 'Failed to open {0} file: {1}.', typeName, (0, common_1.forceError)(e).toString()));
        }
    }
    async sendDifferences(cachedWorldMap, worldMap) {
        await this.progressReporter((0, i18n_1.localize)('worldmap.progress.comparing', 'Comparing changes...'));
        ...
        await this.progressReporter((0, i18n_1.localize)('worldmap.progress.applying', 'Applying changes...'));
    ...
    async requestExportMap() {
        const uri = await vscode.window.showSaveDialog({ filters: { [(0, i18n_1.localize)('pngfile', 'PNG file')]: ['png'] } });
    ...
    async exportMap(dataUrl) {
        ...
        try {
            ...
            await (0, vsccommon_1.writeFile)(uri, buffer);
