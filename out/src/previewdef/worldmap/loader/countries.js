...
class ColorsLoader extends common_1.FileLoader {
    ...
    async loadFromFile() {
        try {
            return {
                result: await (0, fileloader_1.readFileFromModOrHOI4AsJson)(this.file, colorsFileSchema),
                warnings: [],
            };
        }
        catch (e) {
...
async function loadCountryTags(countryTagsFile) {
    try {
        const data = await (0, fileloader_1.readFileFromModOrHOI4AsJson)(countryTagsFile, countryTagsFileSchema);
        const result = [];
...
async function loadCountry(tag, countryFile) {
    try {
        const data = await (0, fileloader_1.readFileFromModOrHOI4AsJson)(countryFile, countryFileSchema);
        return {
            tag,
...
async function applyColorFromColorTxt(countries, colorsFile) {
    for (const country of countries) {
        const colorIncolors = colorsFile._map[country.tag];
        if (colorIncolors?._value.color) {
            country.color = (0, common_1.convertColor)(colorIncolors?._value.color);
        }
    }
}
