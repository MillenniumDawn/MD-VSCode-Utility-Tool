...
class DefaultMapLoader extends common_1.FileLoader {
    ...
    async loadFromFile(session) {
        ...
        const provinceDefinitions = await (this.definitionsLoader = this.checkAndCreateLoader(this.definitionsLoader, 'map/' + defaultMap.definitions, provincedefinitions_1.DefinitionsLoader)).load(session);
        session.throwIfCancelled();
        const provinceBmp = await (this.provinceBmpLoader = this.checkAndCreateLoader(this.provinceBmpLoader, 'map/' + defaultMap.provinces, provincebmp_1.ProvinceBmpLoader)).load(session);
        session.throwIfCancelled();
        const adjacencies = await (this.adjacenciesLoader = this.checkAndCreateLoader(this.adjacenciesLoader, 'map/' + defaultMap.adjacencies, adjacencies_1.AdjacenciesLoader)).load(session);
        session.throwIfCancelled();
        const continents = await (this.continentsLoader = this.checkAndCreateLoader(this.continentsLoader, 'map/' + defaultMap.continent, continents_1.ContinentsLoader)).load(session);
        session.throwIfCancelled();
        const terrains = await this.terrainDefinitionLoader.load(session);
        session.throwIfCancelled();
        const rivers = await (this.riverLoader = this.checkAndCreateLoader(this.riverLoader, 'map/' + defaultMap.rivers, river_1.RiverLoader)).load(session);
        session.throwIfCancelled();
        ...
        await this.fireOnProgressEvent((0, i18n_1.localize)('worldmap.progress.mergeandvalidateprovince', 'Merging and validating provinces...'));
...
async function loadDefaultMap(progressReporter) {
    await progressReporter((0, i18n_1.localize)('worldmap.progress.loadingdefaultmap', 'Loading default.map...'));
    const defaultMap = await (0, fileloader_1.readFileFromModOrHOI4AsJson)('map/default.map', defaultMapSchema);
