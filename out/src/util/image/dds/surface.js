...
class Surface {
    ...
    height;
    pixelFormat;
    constructor(buffer, offset, length, name, width, height, pixelFormat) {
        this.buffer = buffer;
        this.offset = offset;
        this.length = length;
        this.name = name;
    ...
    getFullRgbaFromRawPixels(pixelFormat) {
        const valueType = pixelFormat.valueType;
        if (valueType === pixelformat_1.PixelValueType.typeless) {
            throw new common_1.UserError("Can't get rgba from typeless pixel value");
        }
        if (valueType === pixelformat_1.PixelValueType.shardedexp) {
            throw new common_1.UserError("Pixel value type shardedexp are not supported to get rgba");
        }
        if (pixelFormat.channelLengthInPixel.some(l => l > 32)) {
            throw new common_1.UserError("Some channel length larger than 32");
        }
        if (valueType === pixelformat_1.PixelValueType.float) {
            if (pixelFormat.channelLengthInPixel.some(l => l !== 32) || pixelFormat.bitsPerPixel % 32 !== 0) {
                throw new common_1.UserError("Pixel value type float supports only 32 bits channel and bitsPerPixel should be multiply of 32");
            }
        }
        const channelFormat = pixelFormat.channelFormat;
        const resultPutter = resultPutters[channelFormat];
        if (resultPutter === undefined) {
            throw new common_1.UserError(`Channel format ${channelFormat} is not supported to get rgba`);
        }
        const normalizer = pixelNormalizers[valueType];
        if (normalizer === undefined) {
            throw new common_1.UserError(`Value type ${valueType} is not supported to get rgba`);
        }
        const length = this.length;
        const channelReader = getChannelReader(this.buffer, this.offset, length, pixelFormat);
        if (channelReader === undefined) {
            throw new common_1.UserError(`Unsupported pixel format to read: ${(0, pixelformat_1.pixelFormatToString)(pixelFormat)}`);
        }
    ...
    getFullRgbaFromCompressedPixels(pixelFormat) {
        const result = new Uint8Array(this.width * this.height * 4);
        const block = new Uint8Array(4 * 4 * 4);
        const length = this.length;
        const buffer = new Uint8Array(this.buffer, this.offset, length);
        const blockSize = (0, pixelformat_1.getBlockSize)(pixelFormat.compressFormat);
        const width = this.width;
        const height = this.height;
        const blocksPerLine = (width + 3) >> 2;
        for (let i = 0, k = 0; i < length; i += blockSize, k++) {
            switch (pixelFormat.compressFormat) {
                case pixelformat_1.CompressFormat.bc1:
                    decompressDXT1(buffer, i, block);
                    break;
                case pixelformat_1.CompressFormat.bc2:
                    decompressDXT3(buffer, i, block);
                    break;
                case pixelformat_1.CompressFormat.bc3:
                    decompressDXT5(buffer, i, block);
                    break;
                default:
                    throw new common_1.UserError("Compress format not implemented yet: bc" + pixelFormat.compressFormat);
            }
...
;
const colors = new Array(8).fill([]).map(v => new Uint8Array(4));
function r5g6b5ToRgb(value, result) {
...
function powerAverage(v1, p1, v2, p2, result) {
    for (let i = 0; i < v1.length; i++) {
        result[i] = Math.floor(v1[i] * p1 + v2[i] * p2);
    }
}
function decompressDXT1(buffer, offset, block, ignoreAlpha = false) {
...
function decompressDXT3(buffer, offset, block) {
...
function decompressDXT5(buffer, offset, block) {
    decompressDXT1(buffer, offset + 8, block, true);
...
function readUnsignedBitsFromBuffer(buffer, offset, bitOffset, bitsLength) {
    let filled = 0;
    let rest = bitsLength;
    let result = 0;
    if (rest >= 8) {
...
const resultPutters = {
    [pixelformat_1.ChannelFormat.rgb]: rgbResultPutter,
    [pixelformat_1.ChannelFormat.rgba]: rgbaResultPutter,
    [pixelformat_1.ChannelFormat.l]: luminanceResultPutter,
    [pixelformat_1.ChannelFormat.la]: luminanceAlphaResultPutter,
    [pixelformat_1.ChannelFormat.a]: alphaResultPutter,
    [pixelformat_1.ChannelFormat.d]: luminanceResultPutter,
    [pixelformat_1.ChannelFormat.r]: rResultPutter,
    [pixelformat_1.ChannelFormat.g]: gResultPutter,
    [pixelformat_1.ChannelFormat.rg]: rgResultPutter,
};
function rResultPutter(result, offset, pixel) {
    result[offset] = pixel[0] * 255;
    result[offset + 1] = 0;
    result[offset + 2] = 0;
    result[offset + 3] = 255;
}
function gResultPutter(result, offset, pixel) {
    result[offset] = 0;
    result[offset + 1] = pixel[0] * 255;
    result[offset + 2] = 0;
    result[offset + 3] = 255;
}
function rgResultPutter(result, offset, pixel) {
    result[offset] = pixel[0] * 255;
    result[offset + 1] = pixel[1] * 255;
    result[offset + 2] = 0;
    result[offset + 3] = 255;
}
function rgbResultPutter(result, offset, pixel) {
    result[offset] = pixel[0] * 255;
    result[offset + 1] = pixel[1] * 255;
    result[offset + 2] = pixel[2] * 255;
    result[offset + 3] = 255;
}
function rgbaResultPutter(result, offset, pixel) {
    result[offset] = pixel[0] * 255;
    result[offset + 1] = pixel[1] * 255;
    result[offset + 2] = pixel[2] * 255;
    result[offset + 3] = pixel[3] * 255;
}
function alphaResultPutter(result, offset, pixel) {
    result[offset] = result[offset + 1] = result[offset + 2] = 255;
    result[offset + 3] = pixel[0] * 255;
}
function luminanceResultPutter(result, offset, pixel) {
    result[offset] = result[offset + 1] = result[offset + 2] = pixel[0];
    result[offset + 3] = 255;
}
function luminanceAlphaResultPutter(result, offset, pixel) {
    result[offset] = result[offset + 1] = result[offset + 2] = pixel[0];
    result[offset + 3] = pixel[1];
}
const pixelNormalizers = {
    [pixelformat_1.PixelValueType.uint]: uintNormalizer,
    [pixelformat_1.PixelValueType.unorm]: uintNormalizer,
    [pixelformat_1.PixelValueType.unorm_srgb]: unormSrgbNormalizer,
    [pixelformat_1.PixelValueType.sint]: sintNormalizer,
    [pixelformat_1.PixelValueType.snorm]: sintNormalizer,
    [pixelformat_1.PixelValueType.float]: floatNormalizer,
};
function uintNormalizer(value, max) {
    return value / max;
}
function sintNormalizer(value, max) {
    const midValue = (max - 1) / 2;
    return value === midValue + 1 ? -1 :
        (value <= midValue ? value / midValue : -(max - value + 1) / midValue);
}
function floatNormalizer(value, max) {
    return value;
}
function unormSrgbNormalizer(value, max) {
    return Math.pow(value / max, 2.2);
}
function getChannelReader(inputBuffer, inputBufferOffset, length, pixelFormat) {
    if (pixelFormat.valueType === pixelformat_1.PixelValueType.float) {
...
function defaultUint8Reader(buffer, offset, bitOffset, channelStart, channelLength, channelMask, rawPixel) {
...
function all8Reader(buffer, offset, bitOffset, channelStart, channelLength, channelMask, rawPixel) {
    for (let i = 0; i < channelLength.length; i++) {
        rawPixel[i] = buffer[offset + i];
    }
}
function all16Reader(buffer, offset, bitOffset, channelStart, channelLength, channelMask, rawPixel) {
    for (let i = 0; i < channelLength.length; i++) {
        rawPixel[i] = buffer[(offset >> 1) + i];
    }
}
function all32Reader(buffer, offset, bitOffset, channelStart, channelLength, channelMask, rawPixel) {
    for (let i = 0; i < channelLength.length; i++) {
        rawPixel[i] = buffer[(offset >> 2) + i];
    }
}
function masked8Reader(buffer, offset, bitOffset, channelStart, channelLength, channelMask, rawPixel) {
    const v = buffer[offset];
    for (let i = 0; i < channelLength.length; i++) {
        rawPixel[i] = (v & channelMask[i]) >>> channelStart[i];
    }
}
function masked16Reader(buffer, offset, bitOffset, channelStart, channelLength, channelMask, rawPixel) {
    const v = buffer[offset >> 1];
    for (let i = 0; i < channelLength.length; i++) {
        rawPixel[i] = (v & channelMask[i]) >>> channelStart[i];
    }
}
function masked32Reader(buffer, offset, bitOffset, channelStart, channelLength, channelMask, rawPixel) {
