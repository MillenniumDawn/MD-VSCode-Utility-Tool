...
exports.workspaceModFilesCache = exports.modFileStatusContainer = void 0;
...
async function checkAndUpdateModFileStatus(modFile) {
    if (modFile === undefined) {
        updateSelectedModFileStatus(undefined);
        return;
    }
    const error = !(await (0, vsccommon_2.isFile)(modFile));
...
async function selectModFile() {
    ...
    if (vscode.workspace.workspaceFolders) {
        for (const workspaceFolder of vscode.workspace.workspaceFolders) {
            ...
            const mods = await exports.workspaceModFilesCache.get(workspaceFolderPath.toString());
    ...
    const selectResult = await vscode.window.showQuickPick(modsList, { placeHolder: (0, i18n_1.localize)('modfile.selectworkingmod', 'Select working mod') });
    if (selectResult) {
        let modPath = selectResult.detail;
        if (selectResult.selectModFile) {
            const result = await vscode.window.showOpenDialog({ filters: { [(0, i18n_1.localize)('modfile.type', 'Mod file')]: ['mod'] } });
            if (result) {
...
async function getWorkspaceModFiles(uriString) {
    const uri = vscode.Uri.parse(uriString);
    const items = await (0, vsccommon_2.readDir)(uri);
