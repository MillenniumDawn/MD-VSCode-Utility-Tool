...
async function renderInstantTextBox(textbox, parentInfo, options) {
    const [x, y, width, height] = (0, common_1.calculateBBox)({ ...textbox, size: { width: textbox.maxwidth, height: textbox.maxheight } }, parentInfo);
    const borderX = (0, common_1.normalizeNumberLike)(textbox.bordersize?.x, width);
    const borderY = (0, common_1.normalizeNumberLike)(textbox.bordersize?.y, height);
    ...
    ">
        ${(0, html_1.htmlEscape)(featureflags_1.localisationIndex ? (await (0, localisationIndex_1.getLocalisedTextQuick)(textbox.text) ?? ' ') : (textbox.text ?? ''))}
    </div>`;
}
