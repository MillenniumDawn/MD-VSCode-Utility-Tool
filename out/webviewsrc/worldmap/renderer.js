...
class Renderer extends event_1.Subscriber {
    ...
    renderProvinceTooltip(province, worldMap) {
        ...
        this.renderTooltip(`
${stateObject?.impassable ? '|r|' + (0, i18n_1.feLocalize)('worldmap.tooltip.impassable', 'Impassable') : ''}
...
${worldMap.getSupplyAreaWarnings(supplyArea).map(v => '|r|' + v).join('\n')}`);
    ...
    renderTooltip(tooltip, sizeCallback, renderCallback) {
        ...
        backCanvasContext.textAlign = 'start';
        let width = (0, lodash_1.max)(text.map(t => backCanvasContext.measureText(t).width)) ?? 0;
...
function findNearestPoints(start, end, a, b) {
    ...
    if (!start) {
        ...
        return nearestPair ?? [(0, graphutils_1.bboxCenter)(a.boundingBox), (0, graphutils_1.bboxCenter)(a.boundingBox)];
    }
    else {
        let nearestPair = undefined;
        let nearestPairDistance = 1e10;
        ...
        case 'terrain':
            {
                ...
                return renderContext.extraState[navalTerrain ?? province.terrain]?.color ?? 0;
            }
        case 'continent':
            if (renderContext.extraState === undefined) {
                let continent = 0;
        ...
        case 'resources':
            {
                ...
                if (renderContext.extraState === undefined) {
                    ...
                    worldMap.forEachState(state => {
                        const numResources = Object.values(state.resources).reduce((p, c) => p + (c ?? 0), 0);
                ...
                const stateId = provinceToState[province.id];
                const state = worldMap.getStateById(stateId);
                const numResources = state ? Object.values(state.resources).reduce((p, c) => p + (c ?? 0), 0) : 0;
