...
class TopBar extends event_1.Subscriber {
    ...
    constructor(canvas, viewPoint, loader, state) {
        ...
        this.viewMode$ = (0, event_1.toBehaviorSubject)(document.getElementById('viewmode'), state.viewMode ?? 'province');
        this.colorSet$ = (0, event_1.toBehaviorSubject)(document.getElementById('colorset'), state.colorSet ?? 'provinceid');
        this.hoverProvinceId$ = new rxjs_1.BehaviorSubject(undefined);
        this.selectedProvinceId$ = new rxjs_1.BehaviorSubject(state.selectedProvinceId ?? undefined);
        this.hoverStateId$ = new rxjs_1.BehaviorSubject(undefined);
        this.selectedStateId$ = new rxjs_1.BehaviorSubject(state.selectedStateId ?? undefined);
        this.hoverStrategicRegionId$ = new rxjs_1.BehaviorSubject(undefined);
        this.selectedStrategicRegionId$ = new rxjs_1.BehaviorSubject(state.selectedStrategicRegionId ?? undefined);
        this.hoverSupplyAreaId$ = new rxjs_1.BehaviorSubject(undefined);
        this.selectedSupplyAreaId$ = new rxjs_1.BehaviorSubject(state.selectedSupplyAreaId ?? undefined);
    ...
    onViewModeChange() {
        ...
        if (colorSetHidden) {
            const newColorset = document.querySelector('#colorset > option:not(*[hidden])')?.value;
    ...
    openMapItem(useHoverValue = false) {
        ...
        if (this.viewMode$.value === 'state') {
            ...
            if (selected) {
                ...
                if (state) {
                    vscode_1.vscode.postMessage({ command: 'openfile', type: 'state', file: state.file, start: state.token?.start, end: state.token?.end });
                }
            }
        }
