...
class WorldMapLoader extends common_1.Loader {
    ...
    loadImpl(session) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            session.throwIfCancelled();
            const countries = yield this.countriesLoader.load(session);
            session.throwIfCancelled();
            const strategicRegions = yield this.strategicRegionsLoader.load(session);
            session.throwIfCancelled();
            const enableSupplyArea = (0, vsccommon_1.getConfiguration)().enableSupplyArea;
            const supplyAreas = enableSupplyArea ?
                yield this.supplyAreasLoader.load(session) :
            ...
            const railways = enableSupplyArea ?
                ...
                yield this.railwayLoader.load(session);
            session.throwIfCancelled();
            const supplyNodes = enableSupplyArea ?
                { warnings: [], result: { supplyNodes: [] }, dependencies: [] } :
                yield this.supplyNodeLoader.load(session);
            session.throwIfCancelled();
            const resources = yield this.resourcesLoader.load(session);
            session.throwIfCancelled();
            ...
            const worldMap = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, provinceMap.result), stateMap.result), strategicRegions.result), supplyAreas.result), railways.result), supplyNodes.result), { resources: resources.result, provincesCount: provinceMap.result.provinces.length, statesCount: stateMap.result.states.length, countriesCount: countries.result.length, strategicRegionsCount: strategicRegions.result.strategicRegions.length, supplyAreasCount: supplyAreas.result.supplyAreas.length, countries: countries.result, railwaysCount: railways.result.railways.length, supplyNodesCount: supplyNodes.result.supplyNodes.length, warnings });
    ...
    getWorldMap(force) {
        const session = new loader_1.LoaderSession(force !== null && force !== void 0 ? force : false);
    ...
    extraMesurements(result) {
        return Object.assign(Object.assign({}, super.extraMesurements(result)), { width: result.result.width, height: result.result.height, provincesCount: result.result.provincesCount, statesCount: result.result.statesCount, countriesCount: result.result.countriesCount, strategicRegionsCount: result.result.strategicRegionsCount, supplyAreasCount: result.result.supplyAreasCount });
    }
    toString() {
