...
function setupContainerWindowToggles(folder: string) {
	...
	const containerWindowVisibilities: Record<string, boolean> =
		getState().containerWindowVisibilities ?? {};
	const toggleVisibilityContentInner = document.getElementById(
		"toggleVisibilityContentInner",
	) as HTMLDivElement;
	const containerWindowName = folder.replace("containerwindow_", "");
	toggleVisibilityContentInner.innerHTML =
		(window as any).containerWindowToggles[containerWindowName]?.content ?? "";
	const checkboxes = document.getElementsByClassName(
		"toggleContainerWindowCheckbox",
	);
		...
		for (let j = 0; j <= i; j++) {
			...
			if (input.id.startsWith(anotherInput.id)) {
				selector =
					...
					normalizeForStyle(
						anotherInput.attributes.getNamedItem("containerWindowName")
							?.value ?? "",
					) +
					" ";
			}
	...
	for (let i = 0; i < checkboxes.length; i++) {
		const input = checkboxes.item(i) as HTMLInputElement;
		input.checked = containerWindowVisibilities[input.id] !== false;
		...
		const checkbox = new Checkbox(
			input,
			input.attributes.getNamedItem("containerWindowName")?.value ?? "",
		);
...
function refreshToggleVisibilityContent() {
	...
	const toggleVisibilityContent = document.getElementById(
		"toggleVisibilityContent",
	) as HTMLDivElement;
	toggleVisibilityContent.style.display = toggleVisibilityContentVisible
		? "block"
		: "none";
