...
export class Renderer extends Subscriber {
	...
	public static renderMapImpl(
		...
		Renderer.renderAllOffsets(viewPoint, mapZone, worldMap.width, (xOffset) =>
			Renderer.renderMapBackground(worldMap, xOffset, renderContext),
		);
				...
				if (regionId !== undefined && !renderedRegions[regionId]) {
					...
					if (region) {
						...
						if (viewMode === "state" && colorSet === "resources") {
							const { width } = Renderer.getResourcesSize(
								region as State,
								0.7,
								16,
							);
	...
	private renderProvinceTooltip(province: Province, worldMap: FEWorldMap) {
		...
		const strategicRegion = worldMap.getStrategicRegionByProvinceId(
			province.id,
		);
...
${worldMap
	...
	private renderProvinceHoverSelection(worldMap: FEWorldMap) {
		let province = worldMap.getProvinceById(
			this.topBar.selectedProvinceId$.value,
		);
		if (province) {
		...
		if (province) {
			if (
				this.topBar.selectedProvinceId$ !== this.topBar.hoverProvinceId$ &&
				this.isMouseHighlightVisible()
			) {
	...
	private renderStateHoverSelection(worldMap: FEWorldMap) {
		...
		this.renderHoverSelection(
			worldMap,
			hover,
			worldMap.getStateById(this.topBar.selectedStateId$.value),
		);
		hover &&
	...
	private renderStrategicRegionHoverSelection(worldMap: FEWorldMap) {
		...
		this.renderHoverSelection(
			...
			hover,
			worldMap.getStrategicRegionById(
				this.topBar.selectedStrategicRegionId$.value,
			),
		);
		hover &&
			this.isTooltipVisible() &&
	...
	private renderSupplyAreaHoverSelection(worldMap: FEWorldMap) {
		...
		const selected = worldMap.getSupplyAreaById(
			this.topBar.selectedSupplyAreaId$.value,
		);
		...
		hover &&
			this.isTooltipVisible() &&
			this.renderSupplyAreaTooltip(hover, worldMap);
	}
		...
		case "continent":
			if (renderContext.extraState === undefined) {
				...
				worldMap.forEachProvince(
					(p) => (
						p.continent > continent ? (continent = p.continent) : 0, false
					),
				);
		...
		case "manpower": {
			...
			if (renderContext.extraState === undefined) {
				...
				worldMap.forEachState(
					(state) => (
						state.manpower > maxManpower ? (maxManpower = state.manpower) : 0,
						false
					),
				);
			...
			if (renderContext.extraState === undefined) {
				...
				worldMap.forEachState((state) =>
					Object.values(state.victoryPoints).forEach((vp) =>
						vp !== undefined && vp > maxVictoryPoint
							? (maxVictoryPoint = vp)
							: 0,
					),
				);
		...
		case "supplyvalue": {
			...
			if (renderContext.extraState === undefined) {
				...
				worldMap.forEachSupplyArea(
					(supplyArea) => (
						supplyArea.value > maxSupplyValue
							? (maxSupplyValue = supplyArea.value)
							: 0,
						false
					),
				);
