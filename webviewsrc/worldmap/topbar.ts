...
export class TopBar extends Subscriber {
	...
	constructor(
		...
		this.addSubscription(
			(this.warningFilter = new DivDropdown(
				document.getElementById("warningfilter") as HTMLDivElement,
				true,
			)),
		);
		this.addSubscription(
			(this.display = new DivDropdown(
				document.getElementById("display") as HTMLDivElement,
				true,
			)),
		);

		this.viewMode$ = toBehaviorSubject<ViewMode>(
			document.getElementById("viewmode") as HTMLSelectElement,
			state.viewMode ?? "province",
		);
		this.colorSet$ = toBehaviorSubject<ColorSet>(
			document.getElementById("colorset") as HTMLSelectElement,
			state.colorSet ?? "provinceid",
		);
		this.hoverProvinceId$ = new BehaviorSubject<number | undefined>(undefined);
		this.selectedProvinceId$ = new BehaviorSubject<number | undefined>(
			state.selectedProvinceId ?? undefined,
		);
		this.hoverStateId$ = new BehaviorSubject<number | undefined>(undefined);
		this.selectedStateId$ = new BehaviorSubject<number | undefined>(
			state.selectedStateId ?? undefined,
		);
		this.hoverStrategicRegionId$ = new BehaviorSubject<number | undefined>(
			undefined,
		);
		this.selectedStrategicRegionId$ = new BehaviorSubject<number | undefined>(
			state.selectedStrategicRegionId ?? undefined,
		);
		this.hoverSupplyAreaId$ = new BehaviorSubject<number | undefined>(
			undefined,
		);
		this.selectedSupplyAreaId$ = new BehaviorSubject<number | undefined>(
			state.selectedSupplyAreaId ?? undefined,
		);
	...
	private onViewModeChange() {
		...
		document
			.querySelectorAll(
				'#colorset > option[viewmode~="' + this.viewMode$.value + '"]',
			)
			.forEach((v) => {
		...
		document
			.querySelectorAll("#colorset > option:not([viewmode])")
			.forEach((v) => {
				if ((v as HTMLOptionElement).value === this.colorSet$.value) {
					colorSetHidden = false;
				}
		...
		document
			.querySelectorAll('button[viewmode~="' + this.viewMode$.value + '"]')
			.forEach((v) => {
		...
		document
			.querySelectorAll('.group[viewmode~="' + this.viewMode$.value + '"]')
			.forEach((v) => {
	...
	private openMapItem(useHoverValue = false) {
		...
		if (this.viewMode$.value === "state") {
			...
			const selected = useHoverValue
				? this.hoverStrategicRegionId$.value
				: this.selectedStrategicRegionId$.value;
			if (selected) {
				const strategicRegion =
					this.loader.worldMap.getStrategicRegionById(selected);
				if (strategicRegion) {
					vscode.postMessage<WorldMapMessage>({
						command: "openfile",
			...
			const selected = useHoverValue
				? this.hoverSupplyAreaId$.value
				: this.selectedSupplyAreaId$.value;
			if (selected) {
	...
	private loadOpenButton() {
		...
		this.addSubscription(
			fromEvent(open, "click").subscribe((e) => {
				e.stopPropagation();
				this.openMapItem();
			}),
		);
		...
		this.addSubscription(
			combineLatest([
				...
				([
					...
					selectedSupplyAreaId,
				]) => {
					open.disabled = !(
						(viewMode === "state" && selectedStateId !== undefined) ||
						(viewMode === "strategicregion" &&
							selectedStrategicRegionId !== undefined) ||
	...
	private loadExportButton() {
		...
		this.addSubscription(
			fromEvent(exportButton, "click").subscribe((e) => {
				e.stopPropagation();
		...
		this.addSubscription(
			fromEvent<MessageEvent>(window, "message").subscribe((event) => {
				...
				const viewPoint = new ViewPoint(canvas, this.loader, 0, {
					x: 0,
					y: 0,
					scale: 1,
				});
	...
	private registerEventListeners(canvas: HTMLCanvasElement) {
		this.addSubscription(
			fromEvent<MouseEvent>(canvas, "mousemove").subscribe((e) => {
				...
				this.hoverStateId$.next(
					this.hoverProvinceId$.value === undefined
						? undefined
						: worldMap.getStateByProvinceId(this.hoverProvinceId$.value)?.id,
				);
				this.hoverStrategicRegionId$.next(
					this.hoverProvinceId$.value === undefined
						? undefined
						: worldMap.getStrategicRegionByProvinceId(
								this.hoverProvinceId$.value,
							)?.id,
				);
				this.hoverSupplyAreaId$.next(
					this.hoverStateId$.value === undefined
						? undefined
						: worldMap.getSupplyAreaByStateId(this.hoverStateId$.value)?.id,
				);
			}),
		);
		...
		this.addSubscription(
			fromEvent(canvas, "click").subscribe(() => {
				switch (this.viewMode$.value) {
					case "province":
						this.selectedProvinceId$.next(
							this.selectedProvinceId$.value === this.hoverProvinceId$.value
								? undefined
								: this.hoverProvinceId$.value,
						);
						break;
					case "state":
						this.selectedStateId$.next(
							this.selectedStateId$.value === this.hoverStateId$.value
								? undefined
								: this.hoverStateId$.value,
						);
						break;
					case "strategicregion":
						this.selectedStrategicRegionId$.next(
							this.selectedStrategicRegionId$.value ===
								this.hoverStrategicRegionId$.value
								? undefined
								: this.hoverStrategicRegionId$.value,
						...
						break;
					case "supplyarea":
						this.selectedSupplyAreaId$.next(
							this.selectedSupplyAreaId$.value === this.hoverSupplyAreaId$.value
								? undefined
								: this.hoverSupplyAreaId$.value,
						);
						break;
		...
		this.addSubscription(
			fromEvent(canvas, "dblclick").subscribe((e) => {
				e.stopPropagation();
	...
	private search(text: string) {
		...
		const [getRegionById, selectedId] =
			viewMode === "province"
				? [this.loader.worldMap.getProvinceById, this.selectedProvinceId$]
				: viewMode === "state"
					? [this.loader.worldMap.getStateById, this.selectedStateId$]
					: viewMode === "strategicregion"
						? [
								this.loader.worldMap.getStrategicRegionById,
								this.selectedStrategicRegionId$,
							]
						: viewMode === "supplyarea"
							? [
									this.loader.worldMap.getSupplyAreaById,
									this.selectedSupplyAreaId$,
								]
							: [() => undefined, undefined];
	...
	private setSearchBoxPlaceHolder(worldMap?: FEWorldMap) {
		...
		switch (this.viewMode$.value) {
			case "province":
				placeholder =
					worldMap.provincesCount > 1 ? `1-${worldMap.provincesCount - 1}` : "";
				break;
			case "state":
				placeholder =
					worldMap.statesCount > 1 ? `1-${worldMap.statesCount - 1}` : "";
				break;
			case "strategicregion":
				placeholder =
					worldMap.strategicRegionsCount > 1
						? `1-${worldMap.strategicRegionsCount - 1}`
						: "";
				break;
			case "supplyarea":
				placeholder =
					worldMap.supplyAreasCount > 1
						? `1-${worldMap.supplyAreasCount - 1}`
						: "";
				break;
			default:
				break;
