...
class Dropdown extends Subscriber {
	...
	private init() {
		this.addSubscription(
			fromEvent<MouseEvent>(this.select, "mousedown").subscribe((e) => {
				e.preventDefault();
				this.select.focus();
		...
		this.addSubscription(
			fromEvent<KeyboardEvent>(this.select, "keydown").subscribe((e) => {
				if (e.code === "Enter") {
					e.preventDefault();
					if (this.closeDropdown) {
						this.closeDropdown();
	...
	private showSelectionsForDropdown() {
		...
		dropdownMenuSubscriptions.push(
			toDisposable(
				dropdownMenu.options$.subscribe((options) => {
		...
		dropdownMenuSubscriptions.push(
			toDisposable(
				dropdownMenu.close$.subscribe((isKey) => {
					if (isKey) {
						this.select.focus();
					}
					this.closeDropdown?.apply(this);
...
export class DivDropdown extends Subscriber {
	...
	constructor(
		...
		this.init();
		this.addSubscription(
			this.selectedValues$.subscribe((value) => {
	...
	private updateSelectedValue(options: Option[]) {
		...
		const valueSpan = this.select.querySelector(
			"span.value",
		) as HTMLSpanElement;
...
class DropdownMenu extends Subscriber {
	...
	public show(host: Element) {
		...
		const selectedOptionIndex = this.multiSelection
			? 0
			: Math.max(
					0,
					this.options.findIndex((o) => o.selected),
				);
	...
	private registerEventHandlerWhenOpen(host: Element) {
		...
		this.subscriptionWhenOpen.push(
		...
		this.subscriptionWhenOpen.push(
			fromEvent<MouseEvent>(window, "focusin").subscribe((e) => {
				if (
					!(
						this.list.contains(e.target as any) ||
						host.contains(e.target as any)
					)
				) {
					closeDropdown();
				}
